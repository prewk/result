...
final class OkTest extends TestCase
{
    ...
    /**
     * @covers \Prewk\Result\Ok::__construct
     * @covers \Prewk\Result\Ok::apply
     */
    public function testApplyWithNonCallable(): void
    {
        $ok = new Ok('Not a callable');

        $this->expectException(ResultException::class);
        $this->expectExceptionMessage('Tried to apply a non-callable to arguments');
     ...
     * @covers \Prewk\Result\Ok::apply
     */
    public function testApplyWithNoArguments(): void
    {
        $ok = new Ok(static fn () => 42);

        $result = $ok->apply();

        self::assertInstanceOf(Ok::class, $result);
        self::assertEquals(42, $result->unwrap());
    }
    ...
    {
        $ok = new Ok(static fn (int $x) => $x * 2);

        $result = $ok->apply(new Ok(10));

        self::assertInstanceOf(Ok::class, $result);
        self::assertEquals(20, $result->unwrap());
    }
     ...
     * @covers \Prewk\Result\Ok::apply
     */
    public function testApplyWithMultipleArguments(): void
    {
        $ok = new Ok(static fn (int $x, int $y) => $x * $y);

        $result = $ok->apply(new Ok(10), new Ok(5));

        self::assertInstanceOf(Ok::class, $result);
        self::assertEquals(50, $result->unwrap());
    }
     ...
     * @covers \Prewk\Result\Ok::apply
     */
    public function testApplyWithErrArgument(): void
    {
        $ok = new Ok(static fn (int $x) => $x * 2);

        $errArg = new Err('Error message');

        $result = $ok->apply($errArg);
